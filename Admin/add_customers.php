	...
	if(isset($_SESSION['admin_login'])) {
		if($_SESSION['admin_login'] == true) {
			...
			if($_SERVER['REQUEST_METHOD'] == "POST") {
				if(!empty($_POST['txtCustomerName'])) {
					$nameHolder = $_POST['txtCutomerName'];
					$resultValidate_name = validate_name($_POST['txtCustomerName']);
				...
				if(!empty($_POST['txtCustomerEmail'])) {
					$emailHolder = $_POST['txtCustomerEmail'];
					$resultValidate_email = validate_email($_POST['txtCustomerEmail']);
				...
				if(!empty($_POST['txtCustomerPhone'])) {
					$phoneHolder = $_POST['txtCustomerPhone'];
					$resultValidate_phone = validate_phone($_POST['txtCustomerPhone']);
				...
				if(!empty($_POST['txtCustomerUname'])) {
					$usernameHolder = $_POST['txtCustomerUname'];
					$resultValidate_username = validate_username($_POST['txtCustomerUname']);
					if($resultValidate_username == 1) {
						$username = $_POST['txtCustomerUname'];
					}
					else{
						$usernameErr = $resultValidate_username;
					}
				}
				if(!empty($_POST['txtCustomerPassword'])) {
					$resultValidate_password = validate_password($_POST['txtCustomerPassword']);
				...
				if($name != null && $email != null && $username != null && $password != null) {
					$query_addCustomer = "INSERT INTO customer(cus_name,cus_email,cus_phone,cus_address,username,password) VALUES('$name','$email','$phone','$address','$username','$password')";
					if(mysqli_query($con,$query_addCustomer)) {
						echo "<script> alert(\"Customer Added Successfully\"); </script>";
						header('Refresh:0');
					}
					else {
						$requireErr = "Adding Customer Failed";
					}
				}
				else {
...
<body>
	...
	<div id="layout-wrapper">
		...
		<div>
			<div class="label-block"> <label for="customer:name">Name</label> </div>
			<div class="input-box"> <input type="text" id="customer:name" name="txtCustomerName" placeholder="Name" value="<?php echo $nameHolder; ?>" required /> </div> <span class="error_message"><?php echo $nameErr; ?></span>
		</div>
		<div>
			<div class="label-block"> <label for="customer:email">Email</label> </div>
			<div class="input-box"> <input type="text" id="customer:email" name="txtCustomerEmail" placeholder="Email" value="<?php echo $emailHolder; ?>" required /> </div> <span class="error_message"><?php echo $emailErr; ?></span>
		</div>
		<div>
			<div class="label-block"> <label for="customer:phone">Phone</label> </div>
			<div class="input-box"> <input type="text" id="customer:phone" name="txtCustomerPhone" placeholder="Phone" value="<?php echo $phoneHolder; ?>" /> </div> <span class="error_message"><?php echo $phoneErr; ?></span>
		</div>
		<div>
			<div class="label-block"> <label for="customer:username">Username</label> </div>
			<div class="input-box"> <input type="text" id="customer:username" name="txtCustomerUname" placeholder="Username" value="<?php echo $usernameHolder; ?>" required /> </div> <span class="error_message"><?php echo $usernameErr; ?></span>
		</div>
		<div>
			<div class="label-block"> <label for="customer:password">Password</label> </div>
			<div class="input-box"> <input type="password" id="customer:password" name="txtCustomerPassword" placeholder="Password" required /> </div> <span class="error_message"><?php echo $passwordErr; ?></span>
		</div>
		<div>
			<input type="submit" value="Add Customer" class="submit_button" /> <span class="error_message"> <?php echo $requireErr; ?> </span><span class="confirm_message"> <?php echo $confirmMessage; ?> </span>
		</div>
       </div>
		</form>
