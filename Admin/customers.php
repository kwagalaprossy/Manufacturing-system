<?php
	...
	if(isset($_SESSION['admin_login'])) {
		if($_SESSION['admin_login'] == true) {
			$query_selectCustomer = "SELECT * FROM customer";
			$result_selectCustomer = mysqli_query($con,$query_selectCustomer);
			if($_SERVER['REQUEST_METHOD'] == "POST") {
				if(isset($_POST['chkId'])) {
					$chkId = $_POST['chkId'];
					foreach($chkId as $id) {
						$query_deleteCustomer = "DELETE FROM customer WHERE cus_id='$id'";
						$result = mysqli_query($con,$query_deleteCustomer);
					}
					if(!$result) {
						echo "<script> alert(\"There was some problems deleting Customer\"); </script>";
						header('Refresh:0');
					}
					else {
...
<body>
	...
	<div id="layout-wrapper">
		...
		<div class="main-content">
			...
			<?php $i=1; while($row_selectCustomer = mysqli_fetch_array($result_selectCustomer)) { ?>
			<tr>
				<td> <input type="checkbox" name="chkId[]" value="<?php echo $row_selectCustomer['cus_id']; ?>" /> </td>
				<td> <?php echo $i; ?> </td>
				<td> <?php echo $row_selectCustomer['cus_name']; ?> </td>
				<td> <?php echo $row_selectCustomer['cus_email']; ?> </td>
				<td> <?php echo $row_selectCustomer['cus_phone']; ?> </td>
				<td> <?php echo $row_selectCustomer['username']; ?> </td>
				<td> <a href="edit_customer.php?id=<?php echo $row_selectCustomer['cus_id']; ?>"><img src="../images/edit.png" alt="edit" /></a> </td>
			</tr>
			<?php $i++; } ?>
		</table>
